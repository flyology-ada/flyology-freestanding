...
temporary_directory=$(mktemp -d /tmp/flyology-m0-verify.XXXXXX)
...
verify_architecture() {
    ...
    primary="build/m0/$architecture/flyology-m0.elf"
    reproduction="$temporary_directory/$architecture/flyology-m0.elf"

    scripts/build-m0.sh "$architecture"
    scripts/build-m0.sh "$architecture" "$temporary_directory/$architecture"
