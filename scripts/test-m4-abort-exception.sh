...
for probe in rendezvous protected; do
    ...
    (
        ...
        "$builder" -q -O0 -gnat2022 -gnata -gnatwa -gnatwe \
            "$repository/probes/m4/$name.adb" -o "$name"
    )
    result=$("$timeout_command" --signal=TERM --kill-after=2s 10s \
        "$output/$name")
    test "$result" = \
        "FLYOLOGY:M4:ABORT_EXCEPTION_BLACK_BOX:PASS:$(printf '%s' "$probe" | tr '[:lower:]' '[:upper:]')"
    printf '%s\n' "$result"
done

echo 'FLYOLOGY:M4:ABORT_EXCEPTION_BLACK_BOX:PASS'
