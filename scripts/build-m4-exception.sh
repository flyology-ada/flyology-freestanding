...
output_directory="build/m4-exception/$architecture"
mkdir -p "$output_directory"
rm -f "$output_directory"/*.ali "$output_directory"/*.o \
      "$output_directory"/b~exception_probe.ad? \
      "$output_directory/flyology-m4-exception.elf"
...
compile_ada() {
    ...
    scripts/toolchain.sh exec "$architecture" "$target-gcc" \
        ...
        -nostdinc -Iruntime/bootstrap -Iruntime/core -Iprobes/m4/interfaces \
...
test -z "$(scripts/toolchain.sh exec "$architecture" \
    "$target-nm" -u "$output_directory/flyology-m4-exception.elf")"
