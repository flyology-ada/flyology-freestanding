...
output_root="$repository/build/probes/m6"
...
for architecture in x86_64 aarch64; do
    ...
    scripts/toolchain.sh exec-at "$architecture" "$output" \
        "$target-gcc" -c "$repository/probes/m6/domain_probe.adb" \
    ...
    scripts/toolchain.sh exec-at "$architecture" "$output" \
        "$target-gcc" -c \
        "$repository/probes/m6/interfaces/s-mudido.ads" \
        -nostdinc \
        -I"$repository/probes/m6/interfaces" \
        -I"$repository/probes/m3/interfaces" \
    ...
    echo "FLYOLOGY:M6:PROBE:PASS:$architecture"
done
