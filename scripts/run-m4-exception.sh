...
test_directory="build/m4-exception/tests/$architecture-smp$cpu_count"
...
image="build/m4-exception/$architecture/flyology-$architecture.fat"
...
status=$?
set -e
case "$status" in 0|124|137) ;; *) exit 1 ;; esac

count_marker() {
    grep -aFo "$1" "$serial_log" | wc -l | tr -d ' '
}
test "$(count_marker 'FLYOLOGY:M4:EXCEPTION:PASS')" -eq 1
test "$(count_marker 'FLYOLOGY:LAST_CHANCE:')" -eq 0
test "$(count_marker 'FLYOLOGY:M4:EXCEPTION:DETAIL:')" -eq 0
...
echo "FLYOLOGY:M4:EXCEPTION_TEST:PASS:$architecture:SMP$cpu_count"
