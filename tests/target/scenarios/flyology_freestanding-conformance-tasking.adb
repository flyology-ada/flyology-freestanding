...
with Flyology_Freestanding.Conformance.Observations;
with Flyology_Freestanding.RTS;
with Flyology_Freestanding.Binder_Support;
with System.Multiprocessors;

package body Flyology_Freestanding.Conformance.Tasking is
   ...
   with Import, Convention => C,
        External_Name => "flyology_freestanding_abort_cleanup_query_count";
   ...
   with Import, Convention => C,
        External_Name => "flyology_freestanding_conformance_report_free_wait_pass";

   procedure Report_Free_Abort_Race_Pass
   with Import, Convention => C,
        External_Name => "flyology_freestanding_conformance_report_free_abort_race_pass";
   ...
   protected body Nested_Ceiling_Probe is
      ...
      begin
         if Flyology_Freestanding.RTS.Current_Active_Priority /= 10 then
   ...
   protected body Ceiling_Probe is
      ...
      begin
         ...
         if Flyology_Freestanding.RTS.Current_Active_Priority /= 8 then
            Ceiling_Check_Failed := True;
         end if;
         Ada.Dynamic_Priorities.Set_Priority (3);
         if Ada.Dynamic_Priorities.Get_Priority /= 3
           or else Flyology_Freestanding.RTS.Current_Active_Priority /= 8
         then
            Ceiling_Check_Failed := True;
         end if;
   ...
   task body Specific_Worker_Type is
      ...
      Core : constant Natural := Flyology_Freestanding.RTS.Current_Core_Number;
   ...
   begin
      if Self = Ada.Task_Identification.Null_Task_Id
        ...
        or else not Flyology_Freestanding.RTS.Validate_Current_Stack
          (Stack_Probe'Address)
      then
         raise Program_Error;
      end if;
      ...
      Flyology_Freestanding.Conformance.Observations.Parallel_Barrier (1);
   ...
   begin
      ...
      Auto_Id (Index) := Self;
      Auto_Core (Index) := Flyology_Freestanding.RTS.Current_Core_Number;
      Flyology_Freestanding.Conformance.Observations.Parallel_Barrier (2);
      delay 0.001;
   ...
   begin
      if not Flyology_Freestanding.RTS.Validate_Current_Stack (Stack_Probe'Address)
      then
         raise Program_Error;
      end if;
      ...
      begin
         ...
         declare
            ...
            Wait_Deadline : constant Ada.Real_Time.Time :=
              Ada.Real_Time.Clock + Ada.Real_Time.Milliseconds (100);
         begin
            while not Flyology_Freestanding.RTS.Any_Free_Wait_Is_Active
            loop
   ...
   begin
      for Scenario in Positive range 1 .. 6 loop
            ...
            begin
               for Attempt in 1 .. 1_000 loop
                  exit when Collision_Call_Started
                    ...
                    and then Flyology_Freestanding.Conformance.Observations.Queued_Call_Count = 1;
                  delay 0.001;
               end loop;
               if not Collision_Call_Started
                 or else Collision_Call_Id /= Client_Id
                 or else Flyology_Freestanding.Conformance.Observations.Queued_Call_Count /= 1
               then
                  Report_Failure;
               end if;
         ...
         if Collision_Server_Accept_Count /= 1
           ...
           or else Flyology_Freestanding.Conformance.Observations.Queued_Call_Count /= 0
   ...
   with Import, Convention => C,
        External_Name => "flyology_freestanding_conformance_report_ordinary_pass";

   procedure Report_Specific_Pass
   with Import, Convention => C,
        External_Name => "flyology_freestanding_conformance_report_specific_pass";

   procedure Report_Parallel_Pass
   with Import, Convention => C,
        External_Name => "flyology_freestanding_conformance_report_parallel_pass";

   procedure Report_Auto_Master_Pass
   with Import, Convention => C,
        External_Name => "flyology_freestanding_conformance_report_auto_master_pass";

   procedure Report_Reclamation_Pass
   with Import, Convention => C,
        External_Name => "flyology_freestanding_conformance_report_reclamation_pass";

   procedure Report_Delay_Pass
   with Import, Convention => C,
        External_Name => "flyology_freestanding_conformance_report_delay_pass";

   procedure Report_Absolute_Delay_Pass
   with Import, Convention => C,
        External_Name => "flyology_freestanding_conformance_report_absolute_delay_pass";

   procedure Report_Protected_Pass
   with Import, Convention => C,
        External_Name => "flyology_freestanding_conformance_report_protected_pass";

   procedure Report_Protected_Entry_Pass
   with Import, Convention => C,
        External_Name => "flyology_freestanding_conformance_report_protected_entry_pass";

   procedure Report_Rendezvous_Pass
   with Import, Convention => C,
        External_Name => "flyology_freestanding_conformance_report_rendezvous_pass";

   procedure Report_Exceptional_Sync_Pass
   with Import, Convention => C,
        External_Name => "flyology_freestanding_conformance_report_exceptional_sync_pass";

   procedure Report_Exceptional_Protected_Immediate_Pass
   with Import, Convention => C,
        External_Name =>
          "flyology_freestanding_conformance_report_exceptional_protected_immediate_pass";

   procedure Report_Exceptional_Protected_Queued_Pass
   with Import, Convention => C,
        External_Name =>
          "flyology_freestanding_conformance_report_exceptional_protected_queued_pass";

   procedure Report_Exception_Abort_Protected_Pass
   with Import, Convention => C,
        External_Name =>
          "flyology_freestanding_conformance_report_exception_abort_protected_pass";

   procedure Report_Exceptional_Rendezvous_Pass
   with Import, Convention => C,
        External_Name => "flyology_freestanding_conformance_report_exceptional_rendezvous_pass";

   procedure Report_Exception_Abort_Pass
   with Import, Convention => C,
        External_Name => "flyology_freestanding_conformance_report_exception_abort_pass";

   procedure Report_Unactivated_Cleanup_Pass
   with Import, Convention => C,
        External_Name => "flyology_freestanding_conformance_report_unactivated_cleanup_pass";

   procedure Report_Allocator_Pass
   with Import, Convention => C,
        External_Name => "flyology_freestanding_conformance_report_allocator_pass";

   procedure Report_Priority_Pass
   with Import, Convention => C,
        External_Name => "flyology_freestanding_conformance_report_priority_pass";

   procedure Report_Ceiling_Pass
   with Import, Convention => C,
        External_Name => "flyology_freestanding_conformance_report_ceiling_pass";

   procedure Report_Conditional_Pass
   with Import, Convention => C,
        External_Name => "flyology_freestanding_conformance_report_conditional_pass";

   procedure Report_Timed_Entry_Pass
   with Import, Convention => C,
        External_Name => "flyology_freestanding_conformance_report_timed_entry_pass";

   procedure Report_Dynamic_Task_Pass
   with Import, Convention => C,
        External_Name => "flyology_freestanding_conformance_report_dynamic_task_pass";

   procedure Report_Free_Task_Pass
   with Import, Convention => C,
        External_Name => "flyology_freestanding_conformance_report_free_task_pass";

   procedure Report_Selective_Wait_Pass
   with Import, Convention => C,
        External_Name => "flyology_freestanding_conformance_report_selective_wait_pass";

   procedure Report_Terminate_Alternative_Pass
   with Import, Convention => C,
        External_Name => "flyology_freestanding_conformance_report_terminate_alternative_pass";

   procedure Report_Abort_Pass
   with Import, Convention => C,
        External_Name => "flyology_freestanding_conformance_report_abort_pass";

   procedure Report_Multi_Abort_Pass
   with Import, Convention => C,
        External_Name => "flyology_freestanding_conformance_report_multi_abort_pass";

   procedure Report_Dependent_Abort_Pass
   with Import, Convention => C,
        External_Name => "flyology_freestanding_conformance_report_dependent_abort_pass";

   procedure Report_Abort_Rendezvous_Pass
   with Import, Convention => C,
        External_Name => "flyology_freestanding_conformance_report_abort_rendezvous_pass";

   procedure Report_Abort_Timeout_Pass
   with Import, Convention => C,
        External_Name => "flyology_freestanding_conformance_report_abort_timeout_pass";

   procedure Report_Abort_Accepted_Pass
   with Import, Convention => C,
        External_Name => "flyology_freestanding_conformance_report_abort_accepted_pass";

   procedure Report_Abort_Protected_Pass
   with Import, Convention => C,
        External_Name => "flyology_freestanding_conformance_report_abort_protected_pass";

   procedure Report_Collision_Stress_Pass
   with Import, Convention => C,
        External_Name => "flyology_freestanding_conformance_report_collision_stress_pass";

   procedure Report_Master_Pass
   with Import, Convention => C,
        External_Name => "flyology_freestanding_conformance_report_master_pass";

   procedure Report_Stack_Pass
   with Import, Convention => C,
        External_Name => "flyology_freestanding_conformance_report_stack_pass";
   ...
   begin
      ...
      begin
         ...
         if not Violation_Caught or else Ceiling_Body_Ran
           ...
           or else Flyology_Freestanding.RTS.Current_Active_Priority /= 9
         then
            Report_Failure;
         end if;
         ...
         if not Ceiling_Body_Ran or else Ceiling_Check_Failed
           ...
           or else Flyology_Freestanding.RTS.Current_Active_Priority /= 3
         then
            Report_Failure;
         end if;
      ...
      Protected_Gate.Close;
      if (Flyology_Freestanding.Binder_Support.Task_Dispatching_Policy = ' '
...
end Flyology_Freestanding.Conformance.Tasking;
