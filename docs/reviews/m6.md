# M6 review — immutable scheduling domains and heterogeneous policies
...
- Closing gate: `scripts/verify-m6.sh`
- Proof gate: `scripts/prove.sh` (FSF GNATprove 16.1.0, level 2)

## Evidence

The serialized closing run ended in `FLYOLOGY:M6:GATE:PASS`. It checked
...
M3 through M6 compiler-interface probes with both target compilers; checked
...
SMP4 M6 campaign five times. Every QEMU run had a 20-second timeout and rejected
...
their M6 markers exactly once. SMP4 requires CPUs 1–2 to remain in the FIFO
...
The M5 pre-Ada FP/SIMD canary flag was per-core. After interrupt-time context
...
Disposition: fixed. The pre-Ada canary remains only in the M2 profile where the
interrupt returns to the same task. M5/M6 use task-local post-resume validation
...
M6 adds no public spawn, fiber, yield, task-creation, scheduler-control, or
...
- Domain membership is immutable in M6. Cross-domain `Assign_Task` and dynamic
...
M6 is complete for immutable scheduling-domain creation and task admission,
