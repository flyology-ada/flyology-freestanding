# M5 review — interrupt-time preemption and standard policies
...
- Closing gate: `scripts/verify-m5.sh`
- Proof gate: `scripts/prove.sh` (FSF GNATprove 16.1.0, level 2)

## Evidence

The frozen reviewed-tree gate ended in `FLYOLOGY:M5:GATE:PASS`. It checked
repository hygiene; proved the deterministic policy and ready-position
kernels; reran the cumulative M3/M4 host models and native M4 black boxes;
...
The M5 model enumerates policy validation, FIFO/round-robin budget decisions,
...
contain their policy-specific M5 marker exactly once. SMP4 additionally requires
...
- Policy semantics: FIFO gives higher-priority preemption and head reinsertion
  ...
  model.
- Memory and lifecycle: M4's off-stack retirement, exact wait generations,
...
- Only whole-partition FIFO and round-robin policy images are closed. Immutable
  scheduling domains and heterogeneous per-domain policies remain M6.
- Deterministic proof/model exploration does not cover all concurrent
  ...
  exploration.
- The fixed M4 execution, identity, stack, handler, and heap capacities remain.
  Shellcheck is unavailable; the repository's shell syntax/hygiene checks pass.

## Decision

M5 is complete for standard-Ada-aligned FIFO and round-robin policy selection,
...
on the two fixed QEMU machine contracts. M6 scheduling domains and heterogeneous
