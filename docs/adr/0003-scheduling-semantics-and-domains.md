...
Domain membership is immutable through dispatching-domain capability. Admission occurs before activation and checks task, requested Ada CPU, and eligible core set. Standard reassignment operations that would move an activated task across domains fail with a documented unsupported-operation diagnostic; Flyology does not claim complete `Assign_Task` support in dispatching-domain capability.
