...
On success, `Flyology_Freestanding.Kernel` accounts the current budget, asks the proved policy
...
only `Flyology_Freestanding.Kernel` changes ownership and only architecture code transfers state.
