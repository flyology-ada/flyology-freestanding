...
Compiler-matched GNAT runtime sources carry `GPL-3.0-or-later WITH GCC-exception-3.1`. The exception is useful for generated target code, but copying runtime units creates a licensing boundary for the runtime library itself. Flyology's original work is intended to remain uniformly MIT/Apache-2.0.
...
Flyology implements the runtime clean-room and copies or adapts no GNAT runtime source, body, comment, or test. Allowed implementation inputs are:
...
- compiler-generated expanded Ada, binder output, ALI metadata, symbol diagnostics, and representation information produced from Flyology-owned test programs; and
...
GCC source archives may remain pinned as toolchain provenance, but scripts and contributors must not use their runtime sources to implement Flyology. If clean-room compatibility proves infeasible for a required semantic, the project stops and records the blocker rather than silently importing GNAT code.
