# M6 dispatching-domain interface record
...
The owned `probes/m6/domain_probe.adb` declares one task with only the standard
`Dispatching_Domain` aspect and one task with both `Dispatching_Domain` and
`CPU`. `scripts/probe-m6-interface.sh` compiles it with `-nostdinc`, the owned
...
of 192 bits and alignment 8. `scripts/probe-m6-interface.sh` obtains and checks
...
M6 retains the Ada 2022 public package shape, including range and `CPU_Set`
...
The compiler-generated `CPU_Set` query uses a secondary-stack return. The M6
...
The authoritative M6 gate combines three deliberately different kinds of
...
deterministic models intentionally do not represent. First, the M5 diagnostic
...
remains in the M2 no-transfer profile, while M5/M6 rely on the task-local
