...
finalizer invokes `Complete_Master`. The freestanding M3 QEMU gate verifies
...
probe. M3 closes only the normal activation/completion/master path. Abnormal
and unactivated cleanup is an explicit M4 gate and remains fail-closed here.
