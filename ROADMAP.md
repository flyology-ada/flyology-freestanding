...
| --- | --- | --- |
| product structure | complete ([review](docs/reviews/product-structure.md)) | Responsibility-owned runtime/library layout, real host and target Alire/GPR project graphs, stable capability profiles, exact clean-room inventories, and zero numbered development-stage names in maintained artifacts. |
| application crate workflow | complete ([review](docs/reviews/application-crates.md)) | A dependent Alire crate supplies one ordinary Ada procedure and automatically receives reproducible x86-64/AArch64 Limine UEFI disks, adjacent TianoCore firmware, and a pinned QEMU runner without target-specific GPR configuration. |
