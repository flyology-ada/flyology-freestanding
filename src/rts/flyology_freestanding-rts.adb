...
package body Flyology_Freestanding.RTS is
   ...
   with Import, Convention => C, External_Name => "flyology_freestanding_current_core";

   function Boot_CPU_Count return System.Address
   with Import, Convention => C,
        External_Name => "flyology_freestanding_platform_boot_cpu_count";
   ...
   with Import, Convention => C, External_Name => "flyology_freestanding_platform_kick_core";

   procedure Parallel_Barrier (Phase : System.Address)
   with Import, Convention => C,
        External_Name => "flyology_freestanding_conformance_parallel_barrier";
   ...
   with Import, Convention => C, External_Name => "flyology_freestanding_raise_abort",
        No_Return;

   procedure Raise_Terminate
   with Import, Convention => C,
        External_Name => "flyology_freestanding_raise_terminate", No_Return;
   ...
   with Import, Convention => C,
        External_Name => "flyology_freestanding_raise_exception_identity", No_Return;
   ...
   with Import, Convention => C,
        External_Name => "flyology_freestanding_exception_task_capacity";

   procedure Release_Exception_Task_Slot (Slot : System.Address)
   with Import, Convention => C,
        External_Name => "flyology_freestanding_exception_release_task_slot";

   procedure Task_Root_Invoke
     (Body_Procedure : System.Tasking.Task_Procedure_Access;
      Discriminants  : System.Address)
   with Export, Convention => C,
        External_Name => "flyology_freestanding_task_root_invoke";
   ...
   with Export, Convention => C,
        External_Name => "flyology_freestanding_test_parallel_barrier";

   function Test_Queued_Call_Count return System.Address
   with Export, Convention => C,
        External_Name => "flyology_freestanding_test_queued_call_count";
   ...
   package Domain_Operations is
      ...
      function Domain_Snapshot_Locked return Domains.Domain_State;
   ...
   procedure Register_Domain_Alias
     (Identifier     : Natural;
      Object_Address : System.Address)
   renames Domain_Operations.Register_Domain_Alias;

   procedure Create_Domain
     (Set            : Domain_CPU_Set;
      Identifier     : out Natural;
      Created        : out Boolean)
   renames Domain_Operations.Create_Domain;

   procedure Freeze_Domains renames Domain_Operations.Freeze_Domains;

   function Domain_CPUs (Identifier : Natural) return Domain_CPU_Set
   renames Domain_Operations.Domain_CPUs;

   function Task_Domain (Item : Task_Id) return Natural
   renames Domain_Operations.Task_Domain;
   ...
   procedure Defer_Abort_Locked (Slot : Task_Slot);

   package Rendezvous_Operations is
   ...
   package body Rendezvous_Operations is separate;

   procedure Call_Simple
     (Target      : Task_Id;
      Entry_Index : System.Tasking.Task_Entry_Index;
      Parameters  : System.Address)
   renames Rendezvous_Operations.Call_Simple;
   ...
   procedure Task_Entry_Call
     (Target      : Task_Id;
      ...
      Mode        : System.Tasking.Call_Modes;
      Accepted    : out Boolean)
   renames Rendezvous_Operations.Task_Entry_Call;
   ...
   procedure Timed_Task_Entry_Call
     (Target      : Task_Id;
      ...
      Mode        : Integer;
      Accepted    : out Boolean)
   renames Rendezvous_Operations.Timed_Task_Entry_Call;

   procedure Accept_Call
     (Entry_Index : System.Tasking.Task_Entry_Index;
      Parameters  : out System.Address)
   renames Rendezvous_Operations.Accept_Call;

   procedure Complete_Rendezvous
   renames Rendezvous_Operations.Complete_Rendezvous;

   procedure Exceptional_Complete_Rendezvous
     (Occurrence : System.Address)
   renames Rendezvous_Operations.Exceptional_Complete_Rendezvous;
   ...
   procedure Selective_Wait
     (Alternatives : System.Tasking.Accept_List_Access;
      ...
      Parameters   : out System.Address;
      Selected     : out System.Tasking.Select_Index)
   renames Rendezvous_Operations.Selective_Wait;
   ...
   begin
      ...
      Planning := Domain_Operations.Domain_Snapshot_Locked;
      ...
      for Index in Members'Range loop
         ...
         begin
            ...
            if Explicit then
               Selected := Domain_Operations.Domain_For_Address_Locked
   ...
   begin
      ...
      Leave_Kernel;
      case Flyology_Freestanding.Binder_Support.Task_Dispatching_Policy is
         when ' ' =>
         ...
         when 'F' =>
            if Flyology_Freestanding.Binder_Support.Time_Slice_Value /= 0 then
               Stop;
            end if;
            Core.Configure_Dispatching
              (Core.FIFO_Within_Priorities, 0);
         when 'R' =>
            if Flyology_Freestanding.Binder_Support.Time_Slice_Value not in -1 | 10_000
            then
               Stop;
            end if;
            Core.Configure_Dispatching
              (Core.Round_Robin_Within_Priorities,
               Core.Binder_Time_Slice
                 (if Flyology_Freestanding.Binder_Support.Time_Slice_Value = -1
                  then 10_000
                  else Flyology_Freestanding.Binder_Support.Time_Slice_Value));
         when others =>
            Stop;
      end case;
      Enter_Master;
